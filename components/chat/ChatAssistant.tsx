...
import { Leaf, MessageSquarePlus, Search, Clock, X } from "lucide-react";
...
export default function ChatAssistant({ onClose }: ChatAssistantProps) {
  ...
  return (
    <div className="flex h-full overflow-hidden bg-background-light rounded-2xl border border-[#E2E8F0] shadow-lg">
      {/* ── LEFT SIDEBAR ────────────────────────────────── */}
      ...
      <main className="flex flex-1 flex-col min-w-0 overflow-hidden relative">
        ...
        {onClose && (
          <div className="md:hidden absolute top-4 right-4 z-10">
            <button
              onClick={onClose}
              className="bg-white rounded-full p-2 shadow-md text-slate-600 hover:text-slate-800 transition"
              aria-label="Close chat"
            >
              <X className="h-5 w-5" />
            </button>
